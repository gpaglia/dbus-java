...
public class ClassBuilderInfo {
  ...
  private List<String> createClassFileContent(boolean _staticClass, Set<String> _otherImports) {
    ...
    Set<String> allImports = new TreeSet<>(getImports());
    ...
    bgn
        .append(classIndent)
        .append("public ")
        .append(_staticClass ? "static " : "")
        .append(getClassType() == ClassType.INTERFACE ? "interface" : "class")
        .append(" ")
        .append(getClassName());
    if (getExtendClass() != null) {
      ...
      bgn
          .append(" extends ")
          .append(getClassName(getExtendClass()));
    }
    if (!getImplementedInterfaces().isEmpty()) {
      bgn
          .append(" implements ")
          .append(getImplementedInterfaces().stream().map(ClassBuilderInfo::getClassName).collect(Collectors.joining(", ")));
  ...
  /**
   * Pojo which represents a class member/fieldor argument.
   *
   * @author hypfvieh
   * @since v3.0.1 - 2018-12-20
   */
