...
import org.freedesktop.dbus.support.XmlUtil;
    ...
    if (_methodElement.hasChildNodes()) {
      ...
      for (Element argElm : methodArgs) {
        ...
        if (argElm.getAttribute("type").contains("(")) { // this argument requires some sort of struct
          ...
          String parentType = buildStructClass(
              structPart,
              _methodElement.getAttribute("name") + "Struct",
              _clzBldr,
              additionalClasses);
      ...
      if (outputArgs.size() > 1) { // multi-value return
        ...
        createTuple(outputArgs, _methodElement.getAttribute("name") + "Tuple", _clzBldr, additionalClasses);
      }
