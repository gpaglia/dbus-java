...
public class SampleClass implements SampleRemoteInterface, SampleRemoteInterface2, Properties {
  ...
  public <T> T dostuff(SampleStruct foo) {
    ...
    if (!"bar".equals(foo.getStringValue()) || foo.getInt32Value().intValue() != 52 || !(foo.getVariantValue().getValue() instanceof Boolean) || !(Boolean) foo.getVariantValue().getValue()
    ) {
