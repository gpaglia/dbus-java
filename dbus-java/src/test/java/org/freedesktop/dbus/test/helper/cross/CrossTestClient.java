...
public class CrossTestClient implements Binding.SampleClient, DBusSigHandler<Binding.SampleSignals.Triggered> {
  ...
  public void test(Class<? extends DBusInterface> iface, Object proxy, String method, Object rv, Object... parameters) {
    try {
      ...
      StringBuilder msg = new StringBuilder("Incorrect return value; sent ( ");
      if (null != parameters) {
        for (Object po : parameters) {
          if (null != po) {
            msg.append(collapseArray(po)).append(",");
          }
        }
      }
      msg = new StringBuilder(msg.toString().replaceAll(".$", ");"));
      msg
          .append(" expected ")
          .append(collapseArray(rv))
          .append(" got ")
          .append(collapseArray(o));
      ...
      if (null != rv && rv.getClass().isArray()) {
        ...
        if (o instanceof Map) {
          ...
          if (a.keySet().size() != b.keySet().size()) {
            fail(iface.getName() + "." + method, msg.toString());
          } else {
      ...
      fail(
          ...
          "Error occurred during execution: " + exIt.getCause().getClass().getName() + " " + exIt.getCause().getMessage()
      );
    } catch (Exception exDbe) {
      exDbe.printStackTrace();
      fail(
          iface.getName() + "." + method,
          "Error occurred during execution: " + exDbe.getClass().getName() + " " + exDbe.getMessage()
      );
    }
  }
  ...
  public static String collapseArray(Object array) {
    ...
    if (array.getClass().isArray()) {
      ...
      for (int i = 0; i < Array.getLength(array); i++) {
        s.append(collapseArray(Array.get(array, i))).append(",");
      }
      ...
      for (Object o : (List<Object>) array) {
        s.append(collapseArray(o)).append(",");
      }
      s = new StringBuilder(s.toString().replaceAll(".$", " }"));
      return s.toString();
      ...
      for (Object o : ((Map<Object, Object>) array).keySet()) {
        s.append(collapseArray(o)).append(" => ").append(collapseArray(((Map<Object, Object>) array).get(o))).append(",");
      }
      s = new StringBuilder(s.toString().replaceAll(".$", " }"));
      return s.toString();
    } else {
  ...
  public void doTests(Peer peer, Introspectable intro, Introspectable rootintro, SamplesInterface tests, Binding.SingleSample singletests) {
    ...
    test(SamplesInterface.class, tests, "Identity", new Variant<>(1), new Variant<>(1));
    ...
    int[] is = new int[0];
    test(SamplesInterface.class, tests, "Sum", 0L, (Object) is);
    r = new Random();
    ...
    test(SamplesInterface.class, tests, "Sum", result, (Object) is);

    byte[] bs = new byte[0];
    test(Binding.SingleSample.class, singletests, "Sum", new UInt32(0), (Object) bs);
    len = (r.nextInt() % 100);
    len = (len < 0 ? -len : len) + 15;
    bs = new byte[len];
    int res = 0;
    ...
    test(Binding.SingleSample.class, singletests, "Sum", new UInt32(res % (UInt32.MAX_VALUE + 1)), (Object) bs);

    test(
        SamplesInterface.class,
        tests,
        "DeStruct",
        new org.freedesktop.dbus.test.helper.interfaces.Binding.Triplet<>(
            "hi",
            new UInt32(12),
            (short) 99
        ),
        new CrossSampleStruct(
            "hi",
            new UInt32(12),
            (short) 99
        )
