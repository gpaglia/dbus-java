...
public abstract class AbstractConnection implements Closeable {
   ...
   *
   * @param _newPoolSize The new number of worker Threads to use.
   */
   ...
   *
   * @param objectpath The path to the object we are exposing. MUST be in slash-notation, like "/org/freedesktop/Local", and
   ...
   * @throws DBusException If the objectpath is already exporting an object. or if objectpath is incorrectly formatted,
   */
   ...
   * this object path.
   *
   * @param _objectPrefix The path below which the fallback handles calls. MUST be in slash-notation, like
   *                      "/org/freedesktop/Local",
   * @param _object       The object to export.
   ...
   * Remove a fallback
   *
   * @param _objectprefix The prefix to remove the fallback for.
   */
   ...
   *
   * @param _objectpath The objectpath to stop exporting.
   */
   ...
   * @throws DBusException thrown in case of errors
   */
  ...
  private void handleMessage(final MethodReturn mr) {
    ...
    if (null != m) {
      ...
      try {
        ...
        LOGGER.error("Error sending message", exDe);
      }
    }
  }
    ...
    try {
      ...
      if (m instanceof MethodCall && e instanceof DBusExecutionException) {
        try {
          ...
          LOGGER.error("Error setting reply", e);
        }
        ...
        try {
          ...
          LOGGER.error("Error setting reply", exDe);
        }
      } else if (m instanceof MethodReturn) {
        try {
          transport.writeMessage(new Error(m, e));
        } catch (IOException | DBusException exIo) {
          LOGGER.debug("Exception writing message", exIo);
        }
      }
