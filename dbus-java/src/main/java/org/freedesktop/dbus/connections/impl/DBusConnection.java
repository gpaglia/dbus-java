...
public final class DBusConnection extends AbstractConnection {
   ...
   *
   * @param _busname The name to release. MUST be in dot-notation like "org.freedesktop.local"
   ...
   *
   * @param _busname The name to respond to. MUST be in dot-notation like "org.freedesktop.local"
  ...
  public String[] getNames() {
    Set<String> names = new TreeSet<>(busnames);
   ...
   *
   * @param _busname    The bus name to connect to. Usually a well known bus name in dot-notation (such as
   ...
   * happens, try specifying the interface explicitly.
   *
   * @param _busname    The bus name to connect to. Usually a well known bus name name in dot-notation (such as
   *                    "org.freedesktop.local") or may be a DBus address such as ":1-16".
   * @param _objectpath The path on which the process is exporting the object.
   * @return A reference to a remote object.
   * @throws ClassCastException If type is not a sub-type of DBusInterface
   ...
   *                    "org.freedesktop.local") or may be a DBus address such as ":1-16".
   * @param _objectpath The path on which the process is exporting the object.$
   * @param _type       The interface they are exporting it on. This type must have the same full class name and exposed
   *                    method signatures as the interface the remote object is exporting.
   * @param _autostart  Disable/Enable auto-starting of services in response to calls on this object. Default is enabled; when
   ...
   * @param _type    The signal to watch for.
   * @param _source  The source of the signal.
   ...
   * @param _type    The signal to watch for.
   * @param _source  The source of the signal.
   * @param _object  The object emitting the signal.
   * @param _handler the handler
   * @throws DBusException      If listening for the signal on the bus failed.
   * @throws ClassCastException If type is not a sub-type of DBusSignal.
   */
   ...
   * @param <T>      class extending {@link DBusSignal}
   * @param _type    The signal to watch for.
   * @param _source  The process which will send the signal. This <b>MUST</b> be a unique bus name and not a well known
   *                 name.
   * @param _handler The handler to call when a signal is received.
   * @throws DBusException      If listening for the signal on the bus failed.
   * @throws ClassCastException If type is not a sub-type of DBusSignal.
   */
   ...
   * @param _source  The process which will send the signal. This <b>MUST</b> be a unique bus name and not a well known
   *                 name.
   * @param _object  The object from which the signal will be emitted
   * @param _handler The handler to call when a signal is received.
   * @throws DBusException      If listening for the signal on the bus failed.
   * @throws ClassCastException If type is not a sub-type of DBusSignal.
   */
    ...
    if (shared) {
      ...
      IDisconnectAction beforeDisconnectAction = () -> {
        ...
        synchronized (exportedObjects) {
          List<String> exportedKeys = exportedObjects
              .keySet()
              .stream()
              .filter(Objects::nonNull)
              .collect(Collectors.toList());
  ...
  private void cleanupPendingCalls(Error _err, boolean _clearPendingCalls) {
