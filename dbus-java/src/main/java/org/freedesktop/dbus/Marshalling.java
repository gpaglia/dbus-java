...
public final class Marshalling {
  ...
  static Object deSerializeParameter(Object _parameter, Type _type, AbstractConnection _conn) throws Exception {
    ...
    if (_type instanceof Class
        && ((Class<?>) _type).isArray()
        && ((Class<?>) _type).getComponentType().equals(Type.class)
        && _parameter instanceof String
    ) {
    ...
    if (_parameter instanceof Object[] || _parameter instanceof List || Objects.requireNonNull(_parameter).getClass().isArray()) {
      if (_type instanceof ParameterizedType) {
  ...
  public static Object[] deSerializeParameters(Object[] _parameters, Type[] _types, AbstractConnection _conn) throws Exception {
    ...
    for (int i = 0; i < _parameters.length; i++) {
      ...
      if ((_types[i] instanceof Class && DBusSerializable.class.isAssignableFrom((Class<?>) _types[i]))
          || (_types[i] instanceof ParameterizedType
          && DBusSerializable.class.isAssignableFrom((Class<?>) ((ParameterizedType) _types[i]).getRawType()))) {
